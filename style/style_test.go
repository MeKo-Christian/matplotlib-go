...
func TestPrecedence_SimulatedFigureAxes(t *testing.T) {
    ...
    // Inherit figure DPI
    if axRC.DPI != 110 { t.Fatalf("expected DPI inherit from figure, got %v", axRC.DPI) }
    // Inherit defaults for untouched fields
    if axRC.LineWidth != Default.LineWidth { t.Fatalf("expected default line width inherit, got %v", axRC.LineWidth) }
}
