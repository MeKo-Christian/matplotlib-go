...
func TestUnparsedKnownRCParamWarns(t *testing.T) {
	...
	// axes.axisbelow is a real matplotlib 3.10.9 rcParam that matplotlib-go
	// does not parse; it must warn instead of vanishing silently.
	_, report, err := ParseMPLStyle("t.mplstyle", "axes.axisbelow: line\n")
	if err != nil {
	...
	if !strings.Contains((*msgs)[0], "axes.axisbelow") {
	...
	if len(report.Unsupported) != 1 || report.Unsupported[0].Key != "axes.axisbelow" {
		t.Errorf("report.Unsupported = %v, want axes.axisbelow", report.Unsupported)
	}
}
