...
func TestUnparsedKnownRCParamWarns(t *testing.T) {
	...
	// text.antialiased is a real matplotlib 3.10.9 rcParam that matplotlib-go
	// does not parse; it must warn instead of vanishing silently.
	_, report, err := ParseMPLStyle("t.mplstyle", "text.antialiased: False\n")
	if err != nil {
	...
	if len(report.Unsupported) != 1 || report.Unsupported[0].Key != "text.antialiased" {
		t.Errorf("report.Unsupported = %v, want text.antialiased", report.Unsupported)
	}
}
...
func TestUnparsedKnownRCParamWarnsOncePerKey(t *testing.T) {
	...
	for range 3 {
		if _, _, err := ParseMPLStyle("t.mplstyle", "text.antialiased: True\n"); err != nil {
			t.Fatalf("ParseMPLStyle() error = %v", err)
		}
	}
