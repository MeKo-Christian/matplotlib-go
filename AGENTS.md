...
- When trying to achieve parity with Matplotlib, always compare with the original code of matplotlib at ./third_party. **`./third_party/matplotlib` and the locally installed matplotlib are both 3.10.9.** You can regenerate/instrument a reference directly: `PYTHONPATH=. python3 test/matplotlib_ref/plots/<id>.py --output-dir /tmp/x` (or `test/matplotlib_ref/generate.py --output-dir <dir>` for all).
  - **MIGRATION IN PROGRESS (started 2026-05-29):** system + third_party were upgraded 3.8.4 → 3.10.9, but `testdata/matplotlib_ref/*.png` were generated by **3.8.4** and have NOT yet been regenerated, so they do not match 3.10.9 yet (every case differs, not just mathtext). Until the reference set is regenerated under 3.10.9, the refs reflect 3.8.4 behavior. Note: 3.10.9 rewrote mathtext internals — its `_genfrac` is axis-height based with per-font TeX-table sub/superscript constants (not the 3.8.4 "="-centered algorithm currently ported in `internal/mathtext/layout.go`), so the mathtext port must be retargeted to 3.10.9 once the refs are regenerated.
