...
- **Renders are memoized — never call `parity.Render` in a catalog driver.** Use `renderCase(id)` (`helpers_test.go`), which renders each ID at most once per test binary and shares the image across `TestGolden`, `TestMatplotlibRef`, and `TestReferenceCompare` — otherwise every case is rasterized three or more times (that was ~85 s of the suite). **Treat the returned image as read-only**; it is shared. Renders must stay deterministic for this to be sound, which the "no unseeded randomness" rule above already requires. The env-gated `diagnostics_*` probes deliberately still call `parity.Render` directly.
- **Catalog subtests run in parallel; parent tests must not.** Every `t.Run` in the six catalog loops calls `t.Parallel()`. The parent `TestGolden`/`TestMatplotlibRef`/`TestReferenceCompare` funcs deliberately do **not** — that is what keeps `-update-golden` correct, since `TestGolden` must finish rewriting a golden before `TestReferenceCompare` reads it. Adding `t.Parallel()` to a parent would introduce a file race. New case-driven assertions should follow the same shape, and be checked with `-race`.
