package fill_basic

import (
	"testing"

	"github.com/cwbudde/matplotlib-go/core"
)

func TestPlotMatchesMatplotlibFillBetweenArgumentOrder(t *testing.T) {
	fig := Plot()
	if len(fig.Children) != 1 {
		t.Fatalf("axes count = %d, want 1", len(fig.Children))
	}
	if len(fig.Children[0].Artists) == 0 {
		t.Fatal("expected fill artist")
	}
	fill, ok := fig.Children[0].Artists[0].(*core.Fill2D)
	if !ok {
		t.Fatalf("first artist = %T, want *core.Fill2D", fig.Children[0].Artists[0])
	}
	if len(fill.Y1) == 0 || fill.Y1[0] != 0 {
		t.Fatalf("lower boundary starts at %v, want Matplotlib fill_between(x, 0, y) order", fill.Y1)
	}
	if len(fill.Y2) == 0 || fill.Y2[0] != 0.5 {
		t.Fatalf("upper boundary starts at %v, want y values", fill.Y2)
	}
}
