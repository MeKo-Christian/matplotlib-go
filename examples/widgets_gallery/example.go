...
func plot(opts ...style.Option) *core.Figure {
	...
	span := widgets.NewSpanSelector(mainAx, "horizontal", widgets.SpanSelectorOptions{})
	span.SetSpan(0.7, 1.35)
	rect := widgets.NewRectangleSelector(mainAx, widgets.RectangleSelectorOptions{})
	rect.SetBounds(geom.Pt{X: 2.25, Y: -0.95}, geom.Pt{X: 3.05, Y: -0.25})
	ellipse := widgets.NewEllipseSelector(mainAx, widgets.EllipseSelectorOptions{})
	ellipse.SetBounds(geom.Pt{X: 3.55, Y: 0.25}, geom.Pt{X: 4.45, Y: 1.00})
	polygon := widgets.NewPolygonSelector(mainAx, widgets.PolygonSelectorOptions{})
	...
	polygon.Close()
	lasso := widgets.NewLassoSelector(mainAx, widgets.LassoSelectorOptions{})
	...
	lasso.Finish()
	cursor := widgets.NewCursor(mainAx, widgets.CursorOptions{})
	...
	pressed := true
	widgets.NewButton(widgets.NewSubplotAxes(outer.Cell(1, 0)), "Apply", widgets.ButtonOptions{Pressed: optional.Of(pressed)})
	widgets.NewSlider(widgets.NewSubplotAxes(outer.Cell(1, 1)), "gain", 0, 1, 0.68, widgets.SliderOptions{})
	widgets.NewRangeSlider(widgets.NewSubplotAxes(outer.Cell(1, 2)), "window", 0, 1, 0.22, 0.78, widgets.RangeSliderOptions{})
	active := true
	widgets.NewTextBox(widgets.NewSubplotAxes(outer.Cell(1, 3)), "label", "phase scan", widgets.TextBoxOptions{Active: optional.Of(active)})
	widgets.NewCheckButtons(widgets.NewSubplotAxes(outer.Span(2, 0, 1, 2)), []string{"signal", "modulation", "grid"}, []bool{true, true, false}, widgets.CheckButtonsOptions{})
	widgets.NewRadioButtons(widgets.NewSubplotAxes(outer.Span(2, 2, 1, 2)), []string{"blue", "amber", "mono"}, 1, widgets.RadioButtonsOptions{})
