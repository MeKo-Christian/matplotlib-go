...
func TestLinearScale_RoundTrip(t *testing.T) {
    ...
    for i := 0; i < 200; i++ {
        ...
        for j := 0; j < 10; j++ {
            ...
            u := s.Fwd(x)
            xr, ok := s.Inv(u)
            if !ok { t.Fatalf("linear inv failed") }
...
func TestAxes2D_RoundTrip(t *testing.T) {
    ...
    for i := 0; i < 100; i++ {
        ...
        for j := 0; j < 10; j++ {
            ...
            q := t2.Apply(p)
            pr, ok := t2.Invert(q)
            if !ok { t.Fatalf("axes2d invert failed") }
...
func TestEdgeCases(t *testing.T) {
    ...
    s := NewLinear(1, 1)
    if _, ok := s.Inv(0.5); ok { t.Fatalf("expected inv=false for degenerate linear domain") }

    // Invalid log params
    badBase := NewLog(1, 10, 1)
    if _, ok := badBase.Inv(0.5); ok { t.Fatalf("expected inv=false for base<=1") }
    badMin := NewLog(0, 10, 10)
    if _, ok := badMin.Inv(0.5); ok { t.Fatalf("expected inv=false for min<=0") }
    badRange := NewLog(5, 5, 10)
    if _, ok := badRange.Inv(0.5); ok { t.Fatalf("expected inv=false for min==max") }
}
