...
func TestDateConverterRCSwitchesDefaultFormatter(t *testing.T) {
	...
	if _, ok := info.Formatter.(AutoDateFormatter); !ok {
		t.Fatalf("default date formatter = %T, want AutoDateFormatter", info.Formatter)
	}
	...
	info = state.axisInfo(0, 30)
	if _, ok := info.Formatter.(ConciseDateFormatter); !ok {
		t.Fatalf("date.converter: concise formatter = %T, want ConciseDateFormatter", info.Formatter)
	}
}
