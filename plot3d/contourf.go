...
//nolint:gocritic // ScalarMapInfo is intentionally copied before layer values are derived.
func contourLayerValues(levels []float64, mapping core.ScalarMapInfo) []float64 {
	if len(levels) < 2 {
		return nil
	}
...
//nolint:gocritic // PlotOptions is an immutable snapshot retained by redraw closures.
func (a *Axes3D) projectedContourFillData(x, y []float64, z [][]float64, alpha float64, opt core.PlotOptions) ([]geom.Path, []render.Color, []float64, float64, core.ScalarMapInfo) {
...
func (a *Axes3D) projectedTriContourFillData(tri core.Triangulation, z []float64, alpha float64, opt core.PlotOptions) ([]geom.Path, []render.Color, []float64, float64, core.ScalarMapInfo) {
	if a == nil {
		return nil, nil, nil, defaultPatchZ, core.ScalarMapInfo{}
	}
	zdir := normalized3DDir(opt.ZDir)
...
func contourTriBandPolygons(tri core.Triangulation, values []float64, low, high float64) [][]geom.Pt {
	polygons := make([][]geom.Pt, 0)
...
func rotatedContourTriangulation(x, y []float64, z [][]float64, zdir string) (core.Triangulation, []float64, bool) {
	rows, cols, ok := validate3DGridContourInput(x, y, z)
	if !ok {
		return core.Triangulation{}, nil, false
	}
	...
	return core.Triangulation{X: pointsX, Y: pointsY, Triangles: triangles, Mask: mask}, values, true
}

//nolint:gocritic // Rotation produces a detached triangulation from a value-semantic input.
func rotatedTriangulation3D(tri core.Triangulation, z []float64, zdir string) (core.Triangulation, []float64, bool) {
	if err := tri.Validate(); err != nil || len(z) != len(tri.X) {
		return core.Triangulation{}, nil, false
	}
	rotated := core.Triangulation{
