// This moved test file intentionally keeps the tightly coupled wireframe,
// surface, trisurf, and voxel behavior family together; splitting it would
// duplicate shared projection and polygon-ordering setup.
//
...
func TestAxes3DWireframeColorsApplyAlphaAndStayNonMappable(t *testing.T) {
	...
	if array := collection.Array(); len(array) != 0 {
...
func TestAxes3DStemColorsApplyAlphaAndStayNonMappable(t *testing.T) {
	...
	if array := container.StemLines.Array(); len(array) != 0 {
		t.Fatalf("stem line scalar array = %v, want non-scalar-mappable LineCollection", array)
	}
	if array := container.MarkerCollection.Array(); len(array) != 0 {
...
func TestAxes3DFillBetweenColorsApplyAlphaAndStayNonMappable(t *testing.T) {
	...
	if array := fill.Array(); len(array) != 0 {
...
func TestAxes3DErrorBarColorsApplyAlphaAndStayNonMappable(t *testing.T) {
	...
	if array := errs.Array(); len(array) != 0 {
...
func TestAxes3DBar3DSingleColorAppliesAlphaShadingAndStaysNonMappable(t *testing.T) {
	...
	if array := edges.Array(); len(array) != 0 {
	...
	if array := faces.Array(); len(array) != 0 {
