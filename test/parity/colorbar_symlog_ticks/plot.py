...
def _symlog_fixture_data(rows, cols):
...
def colorbar_symlog_ticks(out_dir):
    fig, ax = plt.subplots(figsize=(640 / DPI, 360 / DPI), dpi=DPI)
    ax.set_position([0.12, 0.16, 0.78, 0.72])
    ax.set_title("SymLogNorm Colorbar")
    ax.set_xlabel("x")
    ax.set_ylabel("y")

    im = ax.imshow(
        _symlog_fixture_data(5, 7),
        cmap="viridis",
    ...
    save(fig, out_dir, "colorbar_symlog_ticks")


PLOT = colorbar_symlog_ticks
