...
func (s *Slider) Draw(r render.Renderer, ctx *core.DrawContext) {
...
func (s *Slider) Bounds(ctx *core.DrawContext) geom.Rect {
	if s == nil || ctx == nil {
		return geom.Rect{}
	}
	defaults := widgetDefaultsForRC(&ctx.RC)
...
func (s *Slider) Contains(p geom.Pt, ctx *core.DrawContext) (bool, core.PickInfo) {
	if s == nil || ctx == nil {
...
func (s *Slider) ValueForPoint(p geom.Pt, ctx *core.DrawContext) (float64, bool) {
	if s == nil || ctx == nil {
		return 0, false
	}
...
func sliderFraction(minValue, maxValue, value float64) float64 {
...
func clampSliderValue(minValue, maxValue, value float64) float64 {
	if maxValue <= minValue {
		return minValue
	}
...
func normalizeSliderValue(minValue, maxValue, step, value float64) float64 {
	clamped := clampSliderValue(minValue, maxValue, value)
	if maxValue <= minValue {
		return clamped
	}
	if step == 0 {
	...
	based := (clamped - minValue) / step
	return clampSliderValue(minValue, maxValue, minValue+math.Round(based)*step)
}

func mergeSliderOptions(base, override *SliderOptions) {
