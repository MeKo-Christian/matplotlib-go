...
type widgetVisualDefaults struct {
	...
	SliderLabelAlign       core.TextAlign
	SliderValueX           widgetStyleCoordinate
	SliderValueY           widgetStyleCoordinate
	SliderValueAlign       core.TextAlign
	SliderTextVAlign       core.TextVerticalAlign
	...
	TextBoxLabelAlign      core.TextAlign
	TextBoxLabelVAlign     core.TextVerticalAlign
	...
	TextBoxTextAlign       core.TextAlign
	TextBoxTextVAlign      core.TextVerticalAlign
...
func widgetDefaultsForAxes(a *core.Axes) widgetVisualDefaults {
	if a == nil {
		return widgetDefaultsForRC(&style.Default)
	}
	rc := a.ResolvedRC()
	return widgetDefaultsForRC(&rc)
}
...
func widgetDefaultsForRC(rc *style.RC) widgetVisualDefaults {
	...
	case style.WidgetVisualMatplotlib:
		return widgetVisualDefaults{
			...
			SliderLabelAlign:       core.TextAlignRight,
			SliderValueX:           widgetFractionCoord(1.02),
			SliderValueY:           widgetFractionCoord(0.5),
			SliderValueAlign:       core.TextAlignLeft,
			SliderTextVAlign:       core.TextVAlignMiddle,
			SliderTrackXPad:        0,
			SliderTrackYMin:        0.25,
			SliderTrackYMax:        0.75,
			SliderTrackRadius:      0,
			SliderHandleSize:       10 * rc.DPI / 72.0,
			SliderHandleLine:       rc.DPI / 72.0,
			SliderInitColor:        render.Color{R: 1, G: 0, B: 0, A: 1},
			SliderInitLine:         rc.DPI / 72.0,
			RangeTupleText:         true,
			TextBoxPanelPad:        0,
			TextBoxLabelX:          widgetFractionCoord(-0.01),
			TextBoxLabelY:          widgetFractionCoord(0.5),
			TextBoxLabelAlign:      core.TextAlignRight,
			TextBoxLabelVAlign:     core.TextVAlignMiddle,
			TextBoxInputXPad:       0,
			TextBoxInputYMin:       0,
			TextBoxInputYMax:       1,
			TextBoxTextX:           widgetFractionCoord(0.05),
			TextBoxTextY:           widgetFractionCoord(0.5),
			TextBoxTextAlign:       core.TextAlignLeft,
			TextBoxTextVAlign:      core.TextVAlignMiddle,
			...
			CheckBoxMaxSize:        (rc.FontSize / 2) * rc.DPI / 72.0,
			CheckBoxScale:          0.50,
			CheckBoxXPad:           0.15,
			CheckLabelGap:          0.25,
			CheckBoxRadius:         0,
			CheckBoxLineWidth:      rc.DPI / 72.0,
			CheckMarkWidth:         rc.DPI / 72.0,
			CheckMarkStyle:         widgetCheckMarkX,
			RadioCenterXPad:        0.15,
			RadioOuterSize:         (rc.FontSize / 2) * rc.DPI / 72.0,
			RadioInnerSize:         0,
			RadioLineWidth:         rc.DPI / 72.0,
			RadioLabelGap:          0.25,
	...
	default:
		return widgetVisualDefaults{
			...
			SliderLabelAlign:       core.TextAlignLeft,
			SliderValueX:           widgetPixelCoord(-14),
			SliderValueY:           widgetPixelCoord(22),
			SliderValueAlign:       core.TextAlignRight,
			SliderTextVAlign:       core.TextVAlignTop,
			SliderTrackXPad:        14,
			SliderTrackYMin:        -26,
			SliderTrackYMax:        -14,
			SliderTrackRadius:      -1,
			...
			TextBoxLabelAlign:      core.TextAlignLeft,
			TextBoxLabelVAlign:     core.TextVAlignTop,
			TextBoxInputXPad:       4,
			TextBoxInputYMin:       30,
			TextBoxInputYMax:       -8,
...
// NewAxes appends an axes prepared for widget controls.
func NewAxes(f *core.Figure, r geom.Rect, opts ...style.Option) *core.Axes {
	if f == nil {
		return nil
	}
	ax := f.AddAxes(r, opts...)
...
// NewSubFigureAxes appends widget axes inside the subfigure rectangle.
func NewSubFigureAxes(sf *core.SubFigure, r geom.Rect, opts ...style.Option) *core.Axes {
	if sf == nil {
		return nil
	}
	ax := sf.AddAxes(r, opts...)
	prepareWidgetAxes(ax)
	return ax
}

// NewSubplotAxes creates widget axes covering this subplot span.
func NewSubplotAxes(spec core.SubplotSpec, opts ...core.SubplotAxesOption) *core.Axes {
	ax := spec.AddAxes(opts...)
	prepareWidgetAxes(ax)
	return ax
}

func prepareWidgetAxes(a *core.Axes) {
	if a == nil {
		return
	}
	if a.XAxis != nil {
...
func widgetSliderTrackForContext(ctx *core.DrawContext) geom.Rect {
	if ctx == nil {
		return geom.Rect{}
	}
...
func drawCenteredWidgetText(r render.Renderer, ctx *core.DrawContext, center geom.Pt, text string, size float64, color render.Color) {
	drawWidgetText(r, ctx, center, text, size, color, core.TextAlignCenter, core.TextVAlignMiddle)
}

func drawWidgetText(r render.Renderer, ctx *core.DrawContext, anchor geom.Pt, text string, size float64, color render.Color, hAlign core.TextAlign, vAlign core.TextVerticalAlign) {
	core.DrawAlignedText(r, ctx, anchor, text, size, color, hAlign, vAlign)
}
