...
func (e *EllipseSelector) Draw(r render.Renderer, ctx *core.DrawContext) {
	...
	minPoint := geom.Pt{X: e.Min.X, Y: e.Min.Y}
	maxPoint := geom.Pt{X: e.Max.X, Y: e.Max.Y}
	centerData := geom.Pt{X: (minPoint.X + maxPoint.X) / 2, Y: (minPoint.Y + maxPoint.Y) / 2}
	center := ctx.DataToPixel.Apply(centerData)
	p1 := ctx.DataToPixel.Apply(geom.Pt{X: minPoint.X, Y: centerData.Y})
	p2 := ctx.DataToPixel.Apply(geom.Pt{X: maxPoint.X, Y: centerData.Y})
	q1 := ctx.DataToPixel.Apply(geom.Pt{X: centerData.X, Y: minPoint.Y})
	q2 := ctx.DataToPixel.Apply(geom.Pt{X: centerData.X, Y: maxPoint.Y})
...
func (e *EllipseSelector) Bounds(ctx *core.DrawContext) geom.Rect {
...
func (e *EllipseSelector) Contains(p geom.Pt, ctx *core.DrawContext) (bool, core.PickInfo) {
	if e == nil || !e.Active || ctx == nil {
	...
	nx := (data.X - cx) / rx
	ny := (data.Y - cy) / ry
	return nx*nx+ny*ny <= 1, core.PickInfo{}
}

func (e *EllipseSelector) displayRect(ctx *core.DrawContext) geom.Rect {
