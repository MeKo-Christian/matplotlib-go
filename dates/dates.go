...
// Epoch returns the current date epoch, resolving the date.epoch rcParam
...
func SetEpoch(t time.Time) error {
	...
	if !dateEpochState.CompareAndSwap(nil, &utc) {
		return errors.New("dates: SetEpoch must be called before any date conversion or plotting")
	}
	return nil
}
