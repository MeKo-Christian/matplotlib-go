...
var publicSurfaceParityOverrides = []PublicSurfaceParity{
	...
	},
	{
		...
		GoFiles:           []string{"animation/animation.go", "animation/writers.go", "animation/writer_pillow.go", "canvas/scheduler.go"},
		CatalogIDs:        []string{"animation_gallery"},
		Note:              "Go ArtistAnimation-style playback toggles per-frame artist visibility and uses the same stepping, repeat, blit, and GIF-Save infrastructure as FuncAnimation (Phase 17.6.8). External-encoder and GUI-only behavior is intentionally omitted for v1.0: HTML representation and FFmpeg/ImageMagick movie writers (ErrWriterUnsupported).",
	},
	{
	...
	},
	{
		...
		Status:            PublicSurfaceIdiomaticEquivalent,
		GoFiles:           []string{"animation/writers.go"},
		Note:              "Go ports MovieWriterRegistry as the package-level writer registry (RegisterWriter/WriterByName/RegisteredWriters), registering only the deterministic \"pillow\" GIF writer (Phase 17.6.8). Unknown or external-encoder names return ErrWriterUnsupported instead of matplotlib's silent Pillow fallback.",
	},
	{
