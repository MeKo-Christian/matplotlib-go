...
var publicSurfaceParityRules = []publicSurfaceParityRule{
	...
	},
	{
		...
		status:            PublicSurfacePartial,
		closurePhase:      "17.75.4",
		goFiles:           []string{"core/axes3d.go", "core/axes3d_view.go", "core/axes3d_frame.go", "core/axes3d_contour_surface.go", "core/axes3d_bar_voxel.go", "core/axes3d_geometry.go", "core/axes3d_projection.go"},
		...
		note:              "Axes3D construction and common 3D plot helpers exist as typed core APIs with catalog coverage. Phase 17.75.4 depth/clipping work covers line and marker helpers, surfaces, contours, triangulated contours, trisurf, bar3d, fill-between3d, voxels, quiver, stem, and errorbar with computed depth ordering, visible-face or face sorting where applicable, AxLimClip-style explicit-limit clipping, and redraw reprojectors. Axis/view-state work covers Matplotlib 3.10-style view defaults, roll and vertical-axis view_init state, perspective/orthographic projection type and focal-length validation, box-aspect reset behavior, explicit and inverted 3D limits, autoscale margins, active panes, grid/tick styling, tick-label placement, and tick-label visibility toggles. Remaining 3D partial scope is scalar-mappable breadth, dedicated tricontour fixtures, GUI/event methods, Python overload grammar, and residual projection/depth-order differences that are documented as typed Go deviations.",
	},
	{
		idPrefix:          "axis",
		module:            "axis.py",
