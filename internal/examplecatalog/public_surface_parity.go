...
var publicSurfaceParityOverrides = []PublicSurfaceParity{
	...
	},
	{
		...
		GoFiles:           []string{"animation/animation.go", "animation/writers.go", "animation/writer_pillow.go", "animation/writer_apng.go", "animation/writer_ffmpeg.go", "canvas/scheduler.go"},
		CatalogIDs:        []string{"animation_gallery"},
		Note:              "Go FuncAnimation-style playback supports frame stepping, init callbacks, repeat and repeat-delay handling, animated artist tracking, blit-region hooks, event-loop start/stop behavior, and Save with a save-count frame loop (WithSaveCount) writing GIF via GifWriter (Phase 17.6.8), APNG via APNGWriter, plus optional MP4/WebM via -tags ffmpeg. HTML representation and ImageMagick-style writers remain out of scope; cache_frame_data is unnecessary because frames regenerate deterministically.",
	},
	{
		...
		GoFiles:           []string{"animation/animation.go", "animation/writers.go", "animation/writer_pillow.go", "animation/writer_apng.go", "animation/writer_ffmpeg.go", "canvas/scheduler.go"},
		CatalogIDs:        []string{"animation_gallery"},
		Note:              "Go ArtistAnimation-style playback toggles per-frame artist visibility and uses the same stepping, repeat, blit, GIF/APNG Save, and optional -tags ffmpeg MP4/WebM infrastructure as FuncAnimation (Phase 17.6.8 / Phase 6.2). HTML representation and ImageMagick-style writers remain out of scope.",
	},
	{
		...
		FeatureCoverageID: "widgets-events-animation",
		Status:            PublicSurfaceIdiomaticEquivalent,
		GoFiles:           []string{"animation/writers.go", "animation/writer_pillow.go", "animation/writer_apng.go", "animation/writer_ffmpeg.go"},
		Note:              "Go exposes MovieWriter as the writer interface used by GIF, APNG, and optional ffmpeg-backed MP4/WebM writers. It keeps the AbstractMovieWriter-style lifecycle while avoiding Matplotlib's subclass-heavy hierarchy.",
	},
	{
	...
	},
	{
		...
		Status:            PublicSurfaceIdiomaticEquivalent,
		GoFiles:           []string{"animation/writers.go"},
		Note:              "Go ports MovieWriterRegistry as the package-level writer registry (RegisterWriter/WriterByName/RegisteredWriters), registering the deterministic \"pillow\" GIF and \"apng\" writers in all builds and ffmpeg-backed MP4/WebM writers only with -tags ffmpeg. Unknown or unavailable names return ErrWriterUnsupported instead of matplotlib's silent Pillow fallback.",
	},
	{
