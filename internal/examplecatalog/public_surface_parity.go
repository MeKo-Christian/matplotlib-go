...
var publicSurfaceParityOverrides = []PublicSurfaceParity{
	...
	},
	{
		...
		Note:              "Go Animation provides frame stepping, start/stop event-loop integration, repeat handling, animated artist tracking, blit fallback hooks, and Save through the deterministic GifWriter analogue of PillowWriter (Phase 17.6.8). Builds compiled with -tags ffmpeg also provide runtime-detected MP4/WebM shellout writers; default builds keep those names unsupported. HTML representation, frame-cache tuning, APNG, and ImageMagick-style writers remain out of scope.",
	},
	{
	...
	},
	{
		...
		GoFiles:           []string{"animation/animation.go", "animation/writers.go", "animation/writer_pillow.go", "animation/writer_ffmpeg.go", "canvas/scheduler.go"},
		CatalogIDs:        []string{"animation_gallery"},
		Note:              "Go FuncAnimation-style playback supports frame stepping, init callbacks, repeat and repeat-delay handling, animated artist tracking, blit-region hooks, event-loop start/stop behavior, and Save with a save-count frame loop (WithSaveCount) writing GIF via GifWriter (Phase 17.6.8) plus optional MP4/WebM via -tags ffmpeg. HTML representation, APNG, and ImageMagick-style writers remain out of scope; cache_frame_data is unnecessary because frames regenerate deterministically.",
	},
	{
		...
		GoFiles:           []string{"animation/animation.go", "animation/writers.go", "animation/writer_pillow.go", "animation/writer_ffmpeg.go", "canvas/scheduler.go"},
		CatalogIDs:        []string{"animation_gallery"},
		Note:              "Go ArtistAnimation-style playback toggles per-frame artist visibility and uses the same stepping, repeat, blit, GIF-Save, and optional -tags ffmpeg MP4/WebM infrastructure as FuncAnimation (Phase 17.6.8 / Phase 6.2). HTML representation, APNG, and ImageMagick-style writers remain out of scope.",
	},
	{
		...
		FeatureCoverageID: "widgets-events-animation",
		Status:            PublicSurfaceIdiomaticEquivalent,
		GoFiles:           []string{"animation/writers.go", "animation/writer_pillow.go", "animation/writer_ffmpeg.go"},
		Note:              "Go exposes MovieWriter as the writer interface used by GIF and optional ffmpeg-backed MP4/WebM writers. It keeps the AbstractMovieWriter-style lifecycle while avoiding Matplotlib's subclass-heavy hierarchy.",
	},
	{
	...
	},
	{
		...
		Status:            PublicSurfaceIdiomaticEquivalent,
		GoFiles:           []string{"animation/writers.go"},
		Note:              "Go ports MovieWriterRegistry as the package-level writer registry (RegisterWriter/WriterByName/RegisteredWriters), registering the deterministic \"pillow\" GIF writer in all builds and ffmpeg-backed MP4/WebM writers only with -tags ffmpeg. Unknown or unavailable names return ErrWriterUnsupported instead of matplotlib's silent Pillow fallback.",
	},
	{
