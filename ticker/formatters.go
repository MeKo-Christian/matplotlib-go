...
// FormatStep formats x using step to select a stable decimal precision.
func (f ScalarFormatter) FormatStep(x, step float64) string {
	return formatScalarTickLabel(f, x, step)
}

// WithOffsetThreshold returns a copy configured with an explicit offset
// threshold. Unlike assigning OffsetThreshold directly, a zero or negative
// threshold remains distinguishable from the zero-value default.
func (f ScalarFormatter) WithOffsetThreshold(threshold int) ScalarFormatter {
	f.OffsetThreshold = threshold
	f.offsetThresholdSet = true
	return f
}
...
// FormatData formats a standalone data value using the formatter's scientific,
// locale, and MathText settings.
func (f ScalarFormatter) FormatData(value float64) string {
	return scalarFormatData(f, value)
}
...
func (f LogitFormatter) Format(x float64) string {
	...
	label := ""
	switch {
	case approx(2*x, 1, 1e-12):
		if f.OneHalf != "" {
			label = f.OneHalf
		} else {
			label = `\frac{1}{2}`
		}
	case x < 0.5 && isPowerOfTen(x):
		label = fmt.Sprintf("10^{%d}", int(math.Round(math.Log10(x))))
	case x > 0.5 && isPowerOfTen(1-x):
