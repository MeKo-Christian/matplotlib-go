#!/usr/bin/env python3
"""Generate the tolerance audit from the committed parity fixtures.
...
Usage:
    ...
    python3 docs/plans/generate_tolerance_audit.py

Writes ``docs/plans/tolerance-audit.md`` and
``docs/plans/tolerance-audit.json``.
"""
...
OUT_MD = os.path.join(REPO, "docs", "plans", "tolerance-audit.md")
OUT_JSON = os.path.join(REPO, "docs", "plans", "tolerance-audit.json")
...
def measure(row: Row) -> None:
    ...
    # Matches imagecmp: PSNR is a restatement of RMSE.
...
def render_markdown(rows: list[Row]) -> str:
    ...
    w("# Tolerance audit\n")
    w(
        "Generated by `docs/plans/generate_tolerance_audit.py` from the "
    ...
    w(
        "- PSNR is no longer a gate anywhere. Fixing the `uint8` "
        "overflow in `imagecmp`'s PSNR accumulator made `PSNR == "
        ...
        "had made unreachable were dropped, to be re-derived from measurement.\n"
    )
    w("## Why RMSE alone is not a gate\n")
    w(
        "`basic_line` is the smallest plot in the catalog, and it is why this "
        "table exists. Its residual used to be 139 pixels out of "
