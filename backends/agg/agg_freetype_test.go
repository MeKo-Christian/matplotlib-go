...
func TestLongitudeTextMetricsMatchMatplotlibRendererAgg(t *testing.T) {
	r := mustNew(t, 260, 120)
	if r.fontPath == "" {
		t.Skip("Matplotlib comparison needs a path-backed DejaVu Sans font")
	}

	tc := mplTextMetricCase{Text: "longitude", Size: 10, DPI: 100}
	mplMetrics := runMatplotlibTextMetrics(t, r.fontPath, []mplTextMetricCase{tc})
	if len(mplMetrics) != 1 {
		t.Fatalf("matplotlib returned %d metrics, want 1", len(mplMetrics))
	}
	...
	goDescent := math.Max(0, goBounds.Y+goBounds.H)
	if math.Abs(goMetrics.W-mplMetrics[0].Width) > 0.1 {
		t.Fatalf("longitude metrics = width %.4f height %.4f descent %.4f, want matplotlib width %.4f height %.4f descent %.4f",
			goMetrics.W, goBounds.H, goDescent, mplMetrics[0].Width, mplMetrics[0].Height, mplMetrics[0].Descent)
	}
	if math.Abs(goBounds.H-mplMetrics[0].Height) > 0.1 {
		t.Fatalf("longitude metrics = width %.4f height %.4f descent %.4f, want matplotlib width %.4f height %.4f descent %.4f",
			goMetrics.W, goBounds.H, goDescent, mplMetrics[0].Width, mplMetrics[0].Height, mplMetrics[0].Descent)
	}
	if math.Abs(goDescent-mplMetrics[0].Descent) > 0.1 {
		t.Fatalf("longitude metrics = width %.4f height %.4f descent %.4f, want matplotlib width %.4f height %.4f descent %.4f",
			goMetrics.W, goBounds.H, goDescent, mplMetrics[0].Width, mplMetrics[0].Height, mplMetrics[0].Descent)
	}
}

func TestLambertLongitudeTextBaselineMatchesMatplotlibRendererAgg(t *testing.T) {
	r := mustNew(t, 520, 520)
	if r.fontPath == "" {
		t.Skip("Matplotlib comparison needs a path-backed DejaVu Sans font")
	}

	tc := aggTextBaselineCase{
		Name:   "lambert_longitude_xlabel",
		Text:   "longitude",
		Size:   10,
		X:      227.25,
		Y:      35.44444444444444,
	...
	goImg := renderAggTextBaseline(t, r.fontPath, tc, false)
	mplImg, _, _, _ := runMatplotlibTextBaseline(t, r.fontPath, tc, "", 0)
	diff := compareRGBA(goImg, mplImg, 0, 0)
	goBounds, goCount, _ := darkPixelBounds(goImg)
	mplBounds, mplCount, _ := darkPixelBounds(mplImg)

	if goBounds != mplBounds || goCount != mplCount {
		t.Fatalf("Lambert longitude text dark bounds = %v count=%d, want matplotlib %v count=%d; direct mean_abs=%.4f",
			goBounds, goCount, mplBounds, mplCount, diff.MeanAbs)
	}
	if diff.MeanAbs > 0.01 {
		t.Fatalf("Lambert longitude text mean_abs = %.4f, want <= 0.01", diff.MeanAbs)
	}
}
